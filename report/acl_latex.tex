...
Data, graphs and replication instructions are available at: \href{https://github.com/Scrivane/LLM-PROJECT-A5-Analysis-of-Linguistic-Stereotypes-in-Generative-AI}{https://github.com/Scrivane/LLM-PROJECT-A5-Analysis-of-Linguistic-Stereotypes-in-Generative-AI}
...
The phrases used are taken from the (\citet{dataset_napoletano}) dataset and then translated in the other 3 linguistic variety using the tools mentioned above.
For each phrase, adjective , lenguage and role, the prompt was executed only once.
...
To compare roles and understand which one is better at reducing bias , we compute the following metric, in which deltas between Italian and dialect related responses are squared not to take into account the direction of the bias:
\begin{equation}
...
where:  
\begin{itemize}
    \item $y_{d,r,a}$ is the number of ``yes'' responses for adjective $a$ with dialectal phrases for role $r$.
    \item $y_{i,r,a}$ is the number of ``yes'' responses for adjective $a$ with Italian phrases for the same role $r$.
...
To perform this analysis, we used a \texttt{.csv} file containing 141 sentences written in the 4 linguistic varieties, developed with reference to the work of \cite{dataset_napoletano}. Each prompt was only executed once. We then prompted the LLM to give a score to each sentence for all of these six dimensions:
\begin{itemize}
...
All prompt structures were tested using GPT-4.1 mini. \\
...
\begin{table}[h!]
\centering
\caption{Base prompt results. Persona A corresponds to the Italian description, Persona B to the dialectal one.}
\label{tab:baseline}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[h!]
\centering
\caption{Deeper evaluation results with Italian request. Values in brackets represent the variation with respect to the baseline results. Persona A corresponds to the Italian description, Persona B to the dialectal one}
\label{tab:deep_eval_ita}
\begin{tabular}{lccc}
\toprule
\textbf{Language} & \textbf{Persona A} & \textbf{Persona B} \\ \midrule
Average      & 38.24\% & 61.76\% \textbf{\scriptsize\color{gray}(+15.47\%)} \\
...
\end{figure}
From this (\ref{fig:GIO_radar_baseline}) chart, we can clearly see how the LLM tends to be more biased towards certain adjectives. The radar plot shows how dialects are generally perceived more "rural", less educated and less conscientious, while on the other dimensions the LLM doesn't appear to have clear biases. It's interesting to note how the dimension "Friendly" seems to be attributed more to the dialects variation rather than standard italian.
...
\paragraph{R.Q.2.} Our results suggest that prompt style and complexity have a significant impact on the reproduced bias. While with a simpler, zero shot, prompting style the LLM is able to show a more balanced behavior, when adopting a more articulate prompting structure some discrepancies can be observed, likely due to the model ability to adapt to the prompt text language registry (\ref{sec:silvia_results}). Conversely, when employing a Role Prompting approach, we experienced an improvement under almost every aspect: when asked to assume a specific role, the model appeared to provide less stereotypical responses, compared to the role-less analysis.
...
In \ref{sec:silvia_results} we observed that the model consistently presents stereotypical behaviors when prompted with a longer, more sophisticated text.
To extend the outcomes of this research, we suggest exploring more in the depth the asymmetric outcomes between northern and southern regions, to understand whether the reduced bias observed for the Emilian dialect is simply due to data scarcity or if it reflects a cultural stereotype as well. Finally, a significant challenge would be addressing the problem of over-compensation with the 3 agents framework.
...
\section{Extra tables for role prompting analysis in different dialects}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{adriano_role_SIC.png}
    \caption[Trait assignment role]{Divergence in trait assignments for each role in Sicilian , compared to the standard Italian of that role, represented by the central vertical line (zero). }
    \label{fig:trait_role_Sicilian}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{adriano_role_parm.png}
    \caption[Trait assignment role]{Divergence in trait assignments for each role in Parmigiano , compared to the standard Italian of that role, represented by the central vertical line (zero). }
    \label{fig:trait_role_Parmigiano}
\end{figure}
